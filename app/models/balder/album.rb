module Balder
  ...
  scope :untouched, where("balder_albums.id IN ( SELECT DISTINCT balder_photos.album_id FROM balder_photos WHERE balder_photos.description IS NULL AND balder_photos.id NOT IN ( SELECT photo_id FROM balder_photo_tags) )").order('title')
  scope :unused, where("balder_albums.id NOT IN (SELECT album_id FROM balder_collection_albums)")
  scope :used, where("balder_albums.id IN (SELECT album_id FROM balder_collection_albums)")
