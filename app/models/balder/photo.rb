module Balder
  ...
  scope :untouched, :conditions => "balder_photos.description IS NULL AND balder_photos.id NOT IN ( SELECT photo_id FROM balder_photo_tags)", :include => :album
  ...
  def exif_read
    ...
    long = photo.GPSLongitude.split(' ') if photo.GPSLongitude
    if photo.GPSLongitude and long.size > 0
      self.longitude = long[0].to_f + (long[2].to_f*60 + long[3].to_f)/3600
      self.longitude = self.longitude * -1 if long[4]=="W"
      #self.longitude = photo.GPSLongitude if self.longitude.nil?
      lat = photo.GPSLatitude.split(' ')
      self.latitude = lat[0].to_f + (lat[2].to_f*60 + lat[3].to_f)/3600
      self.latitude = self.latitude * -1 if lat[4]=="S"
    end
