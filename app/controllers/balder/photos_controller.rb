...
class PhotosController < Balder::ApplicationController
  ...
  def index
    if params[:tag_id] && params[:album_id]
      ...
      @photos = @tag.photos.find(:all, :conditions => ['balder_photos.album_id = :album', {:album => Album.find(params[:album_id] ) } ], :order => "balder_photos.id ASC")
    elsif params[:tag_id]
      @tag = Tag.find_by_title!( params[:tag_id] )
      @photos = @tag.photos.find(:all, :order => "balder_photos.id ASC")
    elsif params[:album_id]
      @album = Album.find( params[:album_id])
      @photos = @album.photos.find(:all, :order => "balder_photos.id ASC")
    elsif params[:q]
      #search = params[:q]
      ...
      params[:q].split(" AND ").each {|q|
        qphotos = Photo.find(:all, :limit => 20, :conditions => [ "balder_photos.description LIKE :q OR balder_photos.title LIKE :q OR balder_photos.id IN ( SELECT photo_id FROM balder_photo_tags LEFT OUTER JOIN balder_tags ON balder_photo_tags.tag_id = balder_tags.id WHERE balder_tags.title = :t) ", { :q => '%' + q + '%', :t => q } ], :include => :album, :order => "balder_photos.id ASC" )
        if @photos
      ...
      @photos = Photo.find(:all, :order => "balder_photos.id ASC")
    end
