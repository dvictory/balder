module Balder
  class AlbumsController < Balder::ApplicationController
  before_filter :check_public_access
  before_filter :require_role_admin, :only => [:untouched, :new, :create, :edit, :update, :destroy]
  
  def index
    if params[:tag_id]
      @albums = Album.find(:all, :conditions => [ "id IN ( SELECT DISTINCT balder_photos.album_id FROM balder_photos WHERE balder_photos.id IN ( SELECT photo_id FROM balder_photo_tags WHERE balder_photo_tags.tag_id = :q) )", { :q => Tag.find( params[:tag_id] ).id } ], :order => 'title')
    elsif params[:q]
      #search = params[:q]
      ...
      params[:q].split(" AND ").each {|q|
        qphotos = Photo.find(:all, :select => 'DISTINCT album_id', :conditions => [ "description LIKE :q OR title LIKE :q OR id IN ( SELECT photo_id FROM balder_photo_tags LEFT OUTER JOIN balder_tags ON balder_photo_tags.tag_id = balder_tags.id WHERE balder_tags.title LIKE :q)", { :q => '%' + q + '%' } ])
